...
test("main prints .agents help output", async () => {
  const stdout = createBufferedStream();
  const stderr = createBufferedStream();
  ...
  assert.equal(exitCode, 0);
  assert.equal(stderr.read(), "");
  assert.match(stdout.read(), /Writes managed agent files into \.agents/);
...
test("main forwards install options through to installWorkflow", async () => {
  ...
  try {
    ...
    assert.match(stdout.read(), /Install root: \.agents/);
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
  }
});
  ...
  try {
    const exitCode = await main(["install", "--dry-run"], {
      ...
      installWorkflow: async () => ({
        ...
        managedFiles: [
          {
            path: path.join(tempRoot, ".agents", "skills", "gnd-critique", "SKILL.md"),
            status: "updated"
          }
        ],
        conflicts: [
          {
            action: "overwrite",
            relativePath: ".agents/skills/gnd-critique/SKILL.md"
          }
        ]
      })
    });
...
async function installWithManagedConflict(tempRoot, stdout, stderr) {
  const critiquePath = path.join(tempRoot, ".agents", "skills", "gnd-critique", "SKILL.md");
...
test("main can confirm overwrites through the CLI io hook", async () => {
  ...
  try {
    ...
    assert.equal(conflicts[0].relativePath, ".agents/skills/gnd-critique/SKILL.md");
    assert.match(stdout.read(), /updated\s+\.agents\/skills\/gnd-critique\/SKILL\.md/);
